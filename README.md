...
**Note:** The `.task-memory.json` config key `task_prefix` (value `"TASK"`) is unrelated to per-file `Task Prefix:` headers (dev initials). The config key sets the task ID prefix used in the UI; the per-file headers enable team-mode isolation.
