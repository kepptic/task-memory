...
document.addEventListener("alpine:init", () => {
  Alpine.data("taskManager", () => ({
    ...
    async tryRestorePreviousDirectory() {
      const lastSelectedProjectName = localStorage.getItem(
        "lastSelectedProject",
      );
      ...
      if (projectToRestore) {
        try {
          ...
          if (
            await window.fileSystem.verifyPermission(projectToRestore.handle)
          ) {
    ...
    async selectFolder() {
      try {
        ...
        if (error.name !== "AbortError") {
          this.$store.ui.showNotification(this.t("notif.folderError"), "error");
          console.error(error);
        }
      }
    },
    ...
    getCurrentProjectIndex() {
      if (!this.$store.tasks.directoryHandle) return "";

      const currentProjectName = this.$store.tasks.directoryHandle.name;
      const projects = this.$store.ui.recentProjects;

      const index = projects.findIndex((p) => p.name === currentProjectName);
    ...
    async switchProject(projectIndex) {
      ...
      try {
        ...
        this.$store.ui.showNotification(this.t("notif.projectError"), "error");
      }
    },
    ...
    async renameCurrentProject() {
      ...
      const currentProject = projects.find(
        (p) => p.handle === this.$store.tasks.directoryHandle,
      );
      ...
      if (newName && newName.trim()) {
        ...
        if (
          await window.fileSystem.renameProject(projectIndex, newName.trim())
        ) {
          this.$store.ui.showNotification(
    ...
    handleDrop(e, columnId) {
      ...
      this.$store.ui.showNotification(this.t("notif.taskMoved"), "info");
